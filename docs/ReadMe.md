...
<pre>
This script is designed to process PDF files to recover digital text and selectively remove black redaction boxes. It works by creating a new document and re-inserting only the essential elements from the original.
...
```text
pymupdf==1.23.26
Key Observations for your script:The Help Command: Your code specifically looks for ./docs/Readme.md. If you just keep the README in the root folder, the -h flag will return "Help file not found." I've noted this in the documentation above.Dependencies: Only PyMuPDF is required. The other imports (os, glob, argparse, sys) are part of the Python Standard Library.
</pre>